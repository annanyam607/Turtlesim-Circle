...
class SimpleActionServerTurtle:
    ...
    def __init__(self):
        ...
        self._sas = actionlib.SimpleActionServer('/action_turtle',myActionMsgAction,execute_cb=self.func_on_rx_goal,auto_start=False)
        '''
    ...
    def func_move_straight(self, param_dis, param_speed):
        ...
        obj_velocity_mssg.linear.x = param_speed
        # obj_velocity_mssg.angular.z = math.radians(abs(int(param_speed)))  # Anticlockwise
       
        obj_velocity_mssg.angular.z = param_speed/(param_dis)
        # obj_velocity_mssg.angular.z = param_speed
        ...
        dis_moved = 0.0
        goal_distance= (2*3.1415926535897* param_dis)


        while not rospy.is_shutdown():
            # Send feedback to the client
            obj_msg_feedback = myActionMsgFeedback()
            current_degree = math.degrees(self._curr_theta)

            if (current_degree>=0):
                # obj_msg_feedback.cur_x = dis_moved
                # obj_msg_feedback.cur_z = self._curr_z
                obj_msg_feedback.cur_theta = self._curr_theta
                self._sas.publish_feedback(obj_msg_feedback)
                if (round(dis_moved) < goal_distance):
                
                    handle_pub_vel.publish(obj_velocity_mssg)
                    print('Degree Rotated: {}'.format(current_degree))
                    dis_moved = (current_degree/360) * 2 *3.1415926535897 * param_dis
                    # extra_distance=dis_moved
    ...
    #     # Rotate the tandle_pub_vurtle till desired angle is reached
    #     hel = rospy.Publisher(
    ...
    def func_on_rx_goal(self, obj_msg_goal):
        ...
        self.func_move_straight(obj_msg_goal.distance, obj_msg_goal.angle)

        # Send Result to the Client
        obj_msg_result = myActionMsgResult()
        # obj_msg_result.final_x = self._curr_x
        # obj_msg_result.final_z = self._curr_z
